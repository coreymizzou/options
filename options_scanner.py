...
def get_live_option_quote(symbol: str) -> dict:
    """
    Fetch a fresh bid/ask/mid for a single OCC option symbol from Tradier.
    Returns dict with bid, ask, mid keys, or empty dict on failure.
    Used to refresh stale scanner prices before order placement.

    Args:
        symbol: OCC symbol e.g. "GLD260522P00435000"
    """
    if not TRADIER_API_KEY:
        return {}
    try:
...
def find_best_strike(chain_data: dict, exp: str, spot: float, direction: str,
    ...
    try:
        ...
        # Calls: strike between 95% and 112% of spot — no deep OTM
        # Puts:  strike between 88% and 105% of spot — no deep OTM
        # Deep OTM contracts have wide bid/ask and stale prices
        if option_type == "call":
            mono_lo, mono_hi = 0.95, 1.12
        else:
            mono_lo, mono_hi = 0.88, 1.05
        ...
        for c in contracts:
            ...
            # Require live bid AND ask — reject contracts with no live market
            # Using stale "last" price produces wildly wrong entry recommendations
            if bid <= 0 or ask <= 0:
                continue
            ...
            if spread_pct > 0.40:   # tightened from 0.50 — better liquidity required
                continue
        ...
        if best is None:
            ...
            for c in contracts:
                ...
                # Require live bid AND ask — reject stale last-price contracts
                if bid <= 0 or ask <= 0:
                    continue
                mid = round((bid + ask) / 2, 2)
                if mid <= 0:
...
def construct_trade(
    ...
    if "SPREAD" in strategy:
        ...
        # For calls: short_strike > long_strike
        # For puts:  short_strike < long_strike
        ...
        # Find short leg by targeting the calculated strike — NOT by delta
        ...
        if best_exp in chain_data:
            ...
            for c in contracts_list:
                ...
                ask = float(c.get("ask", 0) or 0)
                if bid <= 0 or ask <= 0:
                    continue   # require live bid/ask on short leg too
                mid = round((bid + ask) / 2, 2)
                if mid <= 0:
                    continue
                ...
                if opt_type == "put" and K >= main_leg["strike"]:
                    continue
                # Short leg mid must be less than long leg mid
                if mid >= main_leg.get("mid", 999):
                    continue
        ...
        if not short_leg:
            # Fallback: synthetic short leg at 45% of long leg mid
...
def compute_trade_pricing(trade: dict) -> dict:
    ...
    if "SPREAD" in strategy:
        ...
        for leg in [main, short]:
            ...
            a = leg.get("ask", 0) or 0
            if b > 0 and a > 0:
                leg["mid"] = round((b + a) / 2, 2)

        spread_width = abs(main.get("strike", 0) - short.get("strike", spot))
        debit = round(main.get("mid", 0) - short.get("mid", 0), 2)

        # Hard validation: debit must be positive and less than spread width
        # If not, this is a bad spread — return empty pricing to reject it
        if debit <= 0 or (spread_width > 0 and debit >= spread_width):
            return {}   # caller treats empty pricing as invalid trade
